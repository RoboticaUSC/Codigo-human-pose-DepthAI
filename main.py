...
nm = NNetManager(inputSize=(300, 256))
pm = PipelineManager()
pm.setNnManager(nm)
...
if args.camera:
    fps = FPSHandler()
    pm.createColorCam(previewSize=(300, 256), xout=True)
else:
...
def fracmpier(frame):
    ...
    if keypoints_list is not None and detected_keypoints is not None and personwiseKeypoints is not None:
        ...
        if results.pose_landmarks is not None:
            x1 = int(results.pose_landmarks.landmark[8].x * an)
            y1 = int(results.pose_landmarks.landmark[8].y * al)
    
            x2 = int(results.pose_landmarks.landmark[9].x * an)
            y2 = int(results.pose_landmarks.landmark[9].y * al)
    
            x3 = int(results.pose_landmarks.landmark[10].x * an)
            y3 = int(results.pose_landmarks.landmark[10].y * al)
            p1R = np.array([x1, y1])
            p2R = np.array([x2, y2])
            p3R = np.array([x3, y3])
